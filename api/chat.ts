...
function getGeminiClient(): GoogleGenAI | null {
  ...
  if (!aiClient) {
    aiClient = new GoogleGenAI({
      ...
      httpOptions: {
        headers: {
          'User-Agent': 'fluxglow-vercel-api',
        }
      }
    });
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // 1. Restringe el handler para procesar únicamente peticiones de método POST
  if (req.method !== 'POST') {
  ...
  try {
    ...
    if (!client) {
      console.error("[Flux AI] Fallo de configuración: API Key de Gemini no encontrada en Vercel.");
...
5. **Idioma y Tono**: Responde siempre en español natural, cercano, respetuoso y profundamente humano.`;
    ...
    // 2. Utiliza la lista de modelos oficiales vigentes de Google
    const modelsToAttempt = ["gemini-2.0-flash", "gemini-1.5-flash"];
    let replyText = "";
    ...
    if (!replyText) {
      console.error("[Flux AI] Error interno: No se pudo generar una respuesta con ninguno de los modelos.");
    ...
    // Respuesta con éxito (Estado 200)
    ...
    // 4. Si ocurre un error, registra el detalle en 'console.error' y devuelve un estado 500
